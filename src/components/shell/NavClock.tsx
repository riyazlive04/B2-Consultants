...
/**
 * Both business clocks in the top bar — 🇮🇳 14:32 · 🇩🇪 11:02.
 *
...
export function NavClock() {
  // A minute, not a second. This sits in the app shell; re-rendering it once a second to change
  // nothing would be the most frequent render in the app.
  const { zone, now } = useDualClock(60_000);
  ...
  const Chip = ({ z, className }: { z: Zone; className?: string }) => (
    <span className={`flex items-center gap-1 ${className ?? ""}`}>
      ...
      <span className="tnum">{now ? timeIn(ZONES[z].tz, now) : "--:--"}</span>
